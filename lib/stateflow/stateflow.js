...
function def_system_queries(machine) {
    return {
        _what_can_i_do: function({payload, meta}, cb) {
            ...
            console.log("_what_can_i_do()/machine:", machine);

            const schema = machine.get_schema_marked(this);
            console.log("_what_can_i_do()/schema:", schema);
...
//            const items = ctx.models.Workflow.collection.find({}).toArray();
//            ctx.models.Workflow.find({'ancestors.0': meta.wf_id}).then( (err, items) => {
            ctx.models.Workflow.find({'ancestors.0': meta.wf_id}, (err, items) => {
//            ctx.models.Workflow.find({'ancestors.0': meta.wf_id}).exec().then( (err, items) => {
                console.log("_my_workflows():", {
                    name: "_my_workflows answer",
                    payload,
                    meta,
//                    items
                });
