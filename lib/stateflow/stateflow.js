...
//            return {wf: this, data: this.data, qqq: 'www', cnt: this.cnt, state: this.state};
            return {data: this.data, qqq: 'www', cnt: this.cnt, state: this.state};
//            const schema = machine.get_schema_marked(this);
//            const subschema = schema && schema.roles ? schema.roles[meta.user_role] : null;
...
//            result = this.apply_actions(target, tr.action, payload, ctx);
            this.apply_action(target, tr.action, payload, ctx, cb);
        } else
            cb(null, `no transitions for role '${role_name}' and event '${event_name}'`);
