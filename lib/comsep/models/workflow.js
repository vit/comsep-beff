...
workflowSchema.method({
    ...
    new_child_workflow(type, args) {
        const {payload, ctx, cb} = args;
        ctx.models.Workflow.create_workflow({type, current_workflow: this, payload}, (result, error) => {
            const form_id = payload && payload.form && payload.form._id;
            if(form_id) {
                ctx.models.EventForm.findOne(form_id, (error, form) => {
                    if(form)
                        form.submit({});
                    result.init_created({payload, ctx, cb: (result, error) => {
                        cb(result, error);
                    }});
                });
            } else {
    ...
    init_created: function(args) {
        const {payload, ctx, cb} = args;
//        console.log("init()/machine", this.machine);
//        this.title = 'before initialized'
        this.machine.init_target({target: this, payload, ctx, cb})
    },
